...
You will be asked to input a direction (U for up, L for left and so on)
The unit will move this direction and ask again until you stop it
moveing or run out of movement."""
          .format(name, pos[unit], mob))
    while mob != 0 and keep_moveing == "Y":
        pos = find_p()
        position = list(pos[unit])
        keep_moveing = input("Do you wish to move?(Y or N): ").upper()
        if keep_moveing != "Y" and keep_moveing != "N":
            keep_moveing = input("Do you wish to move?(Y or N): ").upper()
        move = ask_for_move()
